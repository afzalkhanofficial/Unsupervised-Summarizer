...
<header class="relative pt-20 pb-12 overflow-hidden bg-background-dark">
    ...
    <div class="max-w-4xl mx-auto px-4 text-center relative z-10 fade-up">
        ...
        <p class="text-xl text-gray-400 leading-relaxed max-w-2xl mx-auto font-light">
            Upload healthcare policy briefs (PDF, Text, or Images). Our NLP engine extracts entities, categorizes clauses, and generates structured summaries.
        </p>
    </div>
</header>
                ...
                <div class="upload-zone relative w-full h-64 rounded-xl flex flex-col items-center justify-center cursor-pointer group" id="drop-zone">
                    ...
                    <div id="upload-prompt" class="text-center space-y-4 transition-all duration-300 group-hover:scale-105 transform-style-3d">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 font-mono mt-1">Supported: PDF, TXT, PNG, JPG</p>
                        </div>
                    </div>
...
<main class="py-12 px-4 relative">
    ...
    <div class="max-w-7xl mx-auto grid lg:grid-cols-12 gap-8 relative z-10">
      ...
      <section class="lg:col-span-5 space-y-6 fade-up delay-100">
        ...
        <div class="glossary-card bg-surface-dark border border-gray-800 p-1 rounded-xl shadow-lg" data-tilt>
            <div class="bg-[#0d1117] rounded-lg p-4 border border-gray-800 h-[300px] flex flex-col">
                <div class="flex justify-between items-center mb-4 pb-2 border-b border-gray-800">
                     <h2 class="text-xs font-bold text-gray-500 uppercase tracking-widest">Document Preview</h2>
        ...
        <div class="glossary-card bg-surface-dark border border-gray-800 p-0 rounded-xl overflow-hidden flex flex-col h-[500px]" data-tilt>
          <div class="p-4 bg-black/20 border-b border-gray-800 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-afzal-purple to-blue-600 flex items-center justify-center text-white shadow-lg">
                <i class="fa-solid fa-robot text-lg"></i>
            </div>
            <div>
                <h2 class="text-sm font-bold text-white">Med.AI Assistant</h2>
                <p class="text-[10px] text-gray-500 font-mono">CONTEXT AWARE • SECURE</p>
            </div>
          </div>
           
          <div id="chat-panel" class="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-black/10">
             <div class="flex gap-3">
                <div class="w-8 h-8 rounded-full bg-surface-dark border border-gray-700 flex items-center justify-center text-afzal-purple text-xs shrink-0">
                    <i class="fa-solid fa-sparkles"></i>
                </div>
                <div class="bg-surface-dark border border-gray-700 rounded-2xl rounded-tl-none p-3 text-xs text-gray-300 leading-relaxed shadow-sm max-w-[85%]">
                   Analysis complete. I have context on the document above. Ask me about specific figures, dates, or compliance requirements.
                </div>
             </div>
          </div>

          <div class="p-4 bg-surface-dark border-t border-gray-800 relative">
             <input type="text" id="chat-input" class="w-full pl-4 pr-12 py-3.5 rounded-lg bg-[#0a0a0c] border border-gray-700 text-sm text-white focus:outline-none focus:border-afzal-purple focus:ring-1 focus:ring-afzal-purple transition placeholder-gray-600 font-mono" placeholder="Type a query...">
             <button id="chat-send" class="absolute right-6 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-white transition">
...
<script>
    ...
    function addMsg(role, text) {
        ...
        div.className = role === 'user' ? 'flex gap-3 flex-row-reverse fade-up' : 'flex gap-3 fade-up';
        
        const avatar = document.createElement('div');
        avatar.className = `w-8 h-8 rounded-full flex items-center justify-center text-xs shrink-0 border ${role === 'user' ? 'bg-white text-black border-white' : 'bg-surface-dark text-afzal-purple border-gray-700'}`;
        avatar.innerHTML = role === 'user' ? '<i class="fa-solid fa-user"></i>' : '<i class="fa-solid fa-sparkles"></i>';
        
        const bubble = document.createElement('div');
        bubble.className = `max-w-[85%] rounded-2xl p-3 text-xs leading-relaxed border ${role === 'user' ? 'bg-afzal-purple text-white border-afzal-purple rounded-tr-none' : 'bg-surface-dark text-gray-300 border-gray-700 rounded-tl-none'}`;
